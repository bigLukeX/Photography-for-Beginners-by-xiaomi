...
- 主线专题：焦距、曝光、色彩、选片、后期与组图
- 按需专题：真实样张、专业模式工具、简易补光与拍摄伦理
- 高级专题：日式胶片风格，位于第 16 章和第 18 章之间
...
`PracticeCard` 默认显示“20 分钟核心版 / 完整挑战 / 点评后重拍”。只有练习说明页这类不是真正作业的卡片，才传 `tiered={false}`。

真实样张必须同时满足：页面显示作者、许可和原始链接，并同步维护 `docs/IMAGE_CREDITS.md`。示意图负责解释变量，真实样张负责训练判断，不要用其中一种完全替代另一种。

AI 生成的教学图必须在页面明确标注为模拟图，不得描述成真实胶片、冲洗工艺或厂商胶片模拟的测量复刻，也不要加入第三方图片署名表。

新增或替换视觉素材前先读 `docs/VISUAL_AUDIT.md`；生成资产与 prompt 摘要维护在 `docs/AI_IMAGE_ASSETS.md`。焦距、透视、曝光数值、直方图和手机 UI 等证据型内容禁止用生成图代替。
...
侧边栏已经按教学依赖排序，不是纯数字顺序：

1. 起点 → 第 1-3 章
2. 焦距专题 → 第 4-5 章 → 曝光 → 色彩 → 第 10 章
3. 第 6-9 章 → 第 11 章 → 选片 → 后期 → 组图 → 第 12 章
4. 第 17 章 → 第 13-16 章 → 日式胶片专题 → 第 18-19 章

不要在没有重新论证课程依赖的情况下，把第 10 章或第 17 章移回纯编号位置。课程目录、侧边栏和页脚导航必须保持同一顺序。
...
- 新增真实图片但漏掉署名和许可归档
- 把专业模式某个开关写成所有 HyperOS 版本都一定存在；应注明“如果当前版本提供”
